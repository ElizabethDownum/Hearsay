# Task5B — receipt-corrected ordinary feature-link proposal

**Authored-by:** Codex GPT-6, inherited highest controller capability. **Date:** 5 September 2026.
**Status:** complete bounded author proposal; independent focused review required.
**Spec:** docs/design-spec.md:79–89; task-5b-model-premises.md:36–43; Plan11 constraints.
**Base:** committed 09e54582f0a5fc9505e396bd1910c800c7413fff plus the frozen26 virtual map.
**Constraints:** task-5b-feature-links-receipt-correction-constraints.md; original completion
constraints and controller R24/R25 adjudications remain binding.
**Dependencies:** actual Task3/4 → approved Task7A → Task5A with T5A-A1 → R16 then
R17 and Task5A2 → preceding eight Task5B model modules → this atomic fragment.
Reconcile actual predecessors and the separately required physical magic, calendar
and full-model composition before any full Task5B dispatch.

## Supersession and exact delta

This version supersedes only the three-code-fence ordinary fragment in
task-5b-feature-links-completed-author-draft.md (SHA256
22b1b12fa0ba1dede919247bd9b93f7d044fa975290ee0af8ea0af343696b664).
That144-case proposal, report, constraints, validation and independent review remain
frozen evidence. Its historical author completeness claim did not constitute approval;
the independent review subsequently found I1 and required this bounded correction.

The only production-code delta from that144-case proposal is in evidenceArrivals.
It recognizes possible ordinary report children before the wrapper's corroboration
gate. A retained wrapper with no actual receipt chronicle cannot confer an arrival
date or per-item roots; matching projected candidates inside its maximum possible
append span stay unrecorded/null. It no longer permits an old relay hearing to
backdate a still-retained child. The same bounded reservation handles R24's incomplete
corroborated batches. Already-resolved or reserved rows remain skipped, so a reserved
nested network child does not recursively reserve its own imagined children.

The feature-links.ts body is byte-identical to the frozen144 proposal. The complete
test body below is its unchanged48-case feature-link file plus the exact three cases
from the independent review's boundary-additions.txt. The seven original test files
and every original96 assertion remain byte-identical. Permanent total:147 =96+48+3.
The review's separate148th trace diagnostic is retained outside the permanent suite.

R25 is PROVISIONAL in task-5b-missing-receipt-adjudication-2026-09-05.md; R24 is in
task-5b-incomplete-batch-adjudication-2026-09-05.md. Both are proposal-only bounds.
This version adds no runtime state, recording rule, metadata, UI or calendar guess.

## Frozen inputs and production premises

The source snapshot began under this owned validation/node_modules/hearsay-baseline
directory via git archive of09e. All302 original snapshot files remain unchanged.
The26-file frozen map is task-5b-reports-validation/relative-sources.json, SHA256
03234a82fcafa458cfec9828b508f98e62ec1d1076401b94e60a84edd389e9c6.
The executed28-file map preserves25 values, replaces evidence.ts, and adds feature-links.ts
and its test file. Seven old model source bodies and seven old test bodies are exact.
source-inventory.json and snapshot-manifest.json name and hash every source input.

Eleven non-model overlays are historical future prerequisites: Task5A artifact claim
links and report-root recording in sim/types, artifacts, directives/types, transport,
phases and chronicle; narrow Task3 provenance in intel/entry and intel/provenance;
Task5A2/R16/R17 in directives/types, reports, execution and evaluator. The frozen
builders already assemble their overlap. These overlays were never written over live
Task3 magic work. Their speech-only types cannot replace actual future physical unions.
T5A-A1's future séance family membership, actual Task4 and corrected Task7A UI remain
actual-base integration prerequisites outside this validated ordinary fragment.

R20's feature reference remains the actual received network-speech receipt; direct
paper retains its claim ref. Only this pure postgame model follows complete unique
retained per-item roots to one relevant raw observation/witness. Multiple matching
answers and unrelated items confer no guessed questioning or compulsion link.
Work history identifies actual retained beats; later presence at the same venue does
not prove watch work. Recorded digest day remains separate from observation/receipt day.

## Task5B-F1 — atomic ordinary feature and acquisition fold

**Model:** frontier_implementer (Sol/high or higher); independent frontier_reviewer
or higher in a separate thread. Root owns the index and serial production writers.
**Files:** src/sim/debrief/evidence.ts; new src/sim/debrief/feature-links.ts;
new tests/debrief/feature-links.test.ts. This fragment licenses no other production file.
**Goal:** reconstruct ordinary evidence acquisition and proved performed attention
without inventing receipt dates, witnesses, links, or continuous staffing.

The supporting optional parent/item/root fields, their consumer and tests are one
atomic interface unit. All three files below are complete; no helper or placeholder
is deferred. Five forensics fixture helpers are exact AST bodies from committed09e.

**Tests first:** retain the old96 tests and transcribe the complete51-case feature-link
test body below. A missing-module import failure proves wiring only. The isolated
correction RED uses the frozen144 proposal plus the exact reviewer additions and
measures146 pass/1 fail, native exit1. Its failure is the child falsely dated direct
at1980 after removal of only the actual1995 receipt. Corrected GREEN is147/147,
native exit0. Do not commit temporary old production code to manufacture RED.
The earlier WIP122/22 and frozen144 GREEN remain separate immutable evidence.

**Self-checks before editing:** verify all three fence hashes against the executed
green map; exact old144 registry and test-body prefix; exact reviewer addition bytes;
all25 preserved frozen-map values; all302 snapshot hashes and1012 protected input
hashes. Check actual predecessor unions, imports and acquisition branches. A physical
union mismatch requires its separately authored migration, never a historical full-file
overwrite. Confirm root has no competing production writer or overlapping dirty work.

**Acceptance:** all old144 cases plus the exact three reviewer cases. In particular,
missing receipt history leaves wrapper/child unknown; independently restored actual
receipt restores its date; nested reserved envelopes do not recurse; distinct direct
rows retain their own dates; byte-indistinguishable candidates remain unknown; no
reservation crosses the maximum projected append span. Complete corroborated reports,
unique roots, one/two-hop H10 refs, actual questioning/compulsion, real worked watch
beats and later-presence negatives, ownership and retained-day behavior remain intact.

**Author gates:** from repo root, Python -X utf8 with the owned run-owned.py:
red native receipt-red-native; green native receipt-green-native; green preflight
compiler-lint; root lint actual-lint-firing. Existing result labels are immutable;
use new labels for any reviewer rerun. The known Vitest/esbuild ancestor-read restriction
uses exact native escalation, with binary stdout/stderr/native exits retained.

**Production gates after actual-base reconciliation:** npm test; npm run lint;
npm run typecheck; npm run app:build. Focused debrief tests supplement the full suite.
Full integration adds soak/MC when required by Plan11. This147-case isolated result
is not the whole-game suite or a full Task5B completion claim. A mechanical_worker may
later transcribe only hash-identical reviewed files under the same integrity/gate rules.

## Exact file: src/sim/debrief/evidence.ts

UTF8/LF SHA256 `66379e449334049373fa1db50de083532952e553bbbb2073d024c4c795a8ccbd`.
...
export function evidenceArrivals(world: WorldState): EvidenceArrival[] {
  ...
  world.enemy.evidence.forEach((entry, index) => {
    ...
    if (!corroborated || !children.every(({ child }, offset) => {
...
UTF8/LF SHA256 `de0fb4c3840838d98c876c68733b4b773883ce48b132e9479c09e675f20443c3`.
