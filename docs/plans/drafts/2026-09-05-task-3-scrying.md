...
**Authored-by:** native Codex, inherited model binding (the author seat does not expose an enforceable model name). **Date:** 2026-09-05. **Status: PARTIAL — user-requested restart wind-down. NOT DISPATCH-READY.** Production proposal and substantial test bodies are saved; the fair-cop audit extension, positive/negative fair-cop controls, app session replay tests, full file-license reconciliation, and mandatory two-pass author self-review are unfinished. No proposed code was executed or typechecked. Resume from `.superpowers/sdd/task-3-author-report.md` before using any snippet.
...
**Base:** source inspected during Task 1 recovery; controller MUST record the real committed Task 2 completion hash in the dispatch header. No future hash is asserted here. **Order:** Task 1 approved → Task 2 approved → 3A → 3B → 3C → 3D → 3E → independent review.
...
**Calibration:** exact author model unavailable — elevated independent review required. No Claude worker is authorized by this draft. All implementation units use native `frontier_implementer`; independent `frontier_reviewer` capability must be at least the implementer's. This is a novel sensory/reporting boundary, not a mechanical transcription assignment despite the complete code below.
...
Add `provenance?: MagicProvenance` to `IntelEntry`; widen its kind union with `'scene-presence' | 'arcane-residue'`; add `residueId?: string`. All ordinary factories omit these optional keys. Do not add fields to old rows at normalization time.
...
In perception add the following exported types. The residue event is a physical fact only; it has no `seenBy`, delivered state, or principal affiliation.
...
Add `residues?: ResidueEvent[]` to `TickEvents` and `| ResidueObservation` to `Observation`. Factor the existing utterance-object construction into this helper, preserving Task 2's marker. Replace only the old `observations.push({...})` inside the existing utterance loop with `observations.push(utteranceObservation(u, u.addressedTo !== observer));`; do not change its gate.
...
Immediately before the existing `return { observer, tick: events.tick, observations };`, append:
...
describe('live venue sensor', () => {
  ...
  it('residue is visible only to real local observers; report bookkeeping is not an event', () => {
    ...
    expect(observationsAtVenue(sensor, bundle).some((row) => row.kind === 'arcane-residue' as string)).toBe(false);
  });
});
```

**RED:** new imports/types absent; after type work, the boundary tests fail until sensor projection exists. **Checksum:** 3 outside + 3 inside parameter rows plus 3 independent tests = 9 test cases. Do not change legacy perception expectations. This unit temporarily widens Observation; all exhaustive consumers must remain compiling at its commit, so land the type-compatible no-op guards listed under 3C at the same unit if needed, explicitly reporting that compile-only move. Do not invent residue behavior in those guards.

## 3B — paid schedule, live capture, physical trace (60–90 minutes)

**Files:** new `src/sim/magic.ts`; `src/sim/types.ts`, `src/sim/campaign.ts`, `src/sim/rules.ts`, `src/content/economy.ts`, `src/sim/phases.ts`, `app/src/input/actions.ts`, `src/content/terms.ts`; new `tests/sim/scrying.test.ts`. **Model/reviewer:** same frontier floors.

Add `scrying: number` to `EconomyDef`, `scrying: 15` to `STANDARD_ECONOMY`, `scry: 'verb-scry'` to `VERB_TERM`. Add the three term rows in 3D now so the action union is compile-complete. Add a type-only `MagicState` import to world types and `magic?: MagicState` to WorldState; no world initializer changes.
...
In `recordAndIngest` widen the captureEvidence guard to:
...
**Important complete-code boundary:** do not commit a union expansion with temporary casts or unhandled exhaustive branches. Units 3A–3C may be committed as one coherent 90-minute schema/capture chunk if typecheck proves they cannot be separated safely; the controller must license that merge explicitly. Unit size is a delivery target, not permission to commit broken code.

## 3C — lawful residue discovery, retry, evidence and digest (75–90 minutes)

**Files:** new `src/sim/residue.ts`; `src/sim/enemy/state.ts`, `src/sim/enemy/digest.ts`, `src/sim/counterintel.ts`, `src/sim/fieldwork.ts`, `src/sim/directives/types.ts`, `src/sim/directives/field-reports.ts`, `src/sim/directives/transport.ts`; `tests/sim/sketch-faircop.test.ts`, new `tests/sim/scry-residue.test.ts`. **Model/reviewer:** frontier floors.
...
import type { ResidueReceipt } from './enemy/state';
import type { WorldState } from './types';
...
export function ingestEnemyResidue(
  world: WorldState, observation: ReportedResidue, receipt?: ResidueReceipt,
): void {
...
export function physicalResidueAt(
  observation: ReportedResidue, tick: Tick = observation.observedAt,
): ResidueObservation {
  return { kind: 'arcane-residue', tick, venue: observation.venue,
    residueId: observation.residueId, witness: observation.witness };
}
```

`physicalResidueAt` is a shape conversion for tests/chronicle joins, not a capture bypass; production does not call it to invent a local observation. It may be omitted entirely if lint reports no test consumer; the test below uses it.
...
In `directives/field-reports.ts`, import the residue helpers plus `type ResidueReceipt` from `../enemy/state`. In `holdFieldObservation`, after the three existing argument validations but before root construction, add:
...
Keep the common `factRefs` loop after the chain. In `ingestEnemyItem`, add a final parameter `residueReceipt: ResidueReceipt`; immediately after binding `observation`, add:

```ts
  if (observation.kind === 'arcane-residue') {
    ingestEnemyResidue(world, observation, residueReceipt);
    return;
  }
```
...
Inside each suspicious-family loop, replace `const voicings = familyEntries.filter((e) => e.reported !== null);` with the narrow type predicate (residue alone has null speakers):

```ts
    const voicings = familyEntries.filter(
      (e): e is Exclude<EvidenceEntry, { kind: 'arcane-residue' }> => e.reported !== null,
    );
```
...
**Files:** `src/intel/board.ts`, `src/intel/types.ts`, `src/intel/web.ts`, `src/intel/ledger.ts`, `src/intel/report.ts`, `src/intel/codex.ts`, `src/content/terms.ts`, `app/src/main.tsx`, `app/src/panels/EvidenceBoard.tsx`, `app/src/panels/WebViewPanel.tsx`, `app/src/panels/EveningReport.tsx`, `app/src/panels/Codex.tsx`, `assets/manifest.json`, `app/src/assets.ts`, `docs/asset-slots.md`; `tests/intel/magic-provenance.test.ts`, new `tests/app/magic-provenance.test.tsx`. **Model/reviewer:** frontier floors; exact vocabulary/manifest edits may be a mechanical sub-unit only if the controller explicitly dispatches it serially.
...
3. `ledger.ts`: import `isMagic`. First loop's guard becomes `if (isMagic(e) || e.via !== via) return;`. The other-channel loop's guard becomes `if (isMagic(e) || e.family === null || e.via === via || !ownFamilies.has(e.family)) return;`. Keep API/result exactly unchanged. An NPC called `scrying` gets only its actual reports; magic does not appear in `otherVias`.
...
```ts
  ...
  })).values()].sort((a, b) => a.label.localeCompare(b.label) || a.key.localeCompare(b.key));
```
...
8. Add these term rows to TERMS (the first three can land with 3B):

```ts
  'verb-scry': { id: 'verb-scry', label: 'Scry', short: 'Buy one remote scene tomorrow: a 15-aligned window of at most an hour. Costs 15 coin and leaves physical residue.', entry: null },
  'scrying': { id: 'scrying', label: 'Scrying', short: 'A purchased view of one venue as events happen. It shows the words faithfully; the words themselves may be false.', entry: null },
  'magic': { id: 'magic', label: 'Magic', short: 'Paid observation through a ritual. It is not an informant and supplies no independent informant channel.', entry: null },
  'scene-presence': { id: 'scene-presence', label: 'Seen in the scene', short: 'A person visible at the purchased venue and time. Presence alone does not say they serve the watch.', entry: null },
  'arcane-residue': { id: 'arcane-residue', label: 'Arcane residue', short: 'A physical trace left where a scrying window begins. Local witnesses may report it; it does not name the caster.', entry: null },
```
...
## 3E — mechanism tests, fair-cop controls, replay and final gate (75–90 minutes)

**Files:** `tests/sim/helpers/scry-world.ts` (new), `tests/sim/scrying.test.ts` (new), `tests/sim/scry-residue.test.ts` (new), `tests/sim/sketch-faircop.test.ts`, `tests/intel/magic-provenance.test.ts`, `tests/app/magic-session.test.ts` (new). **Model/reviewer:** frontier floors. Test files land with the mechanism unit they make RED; this unit closes integration and runs the final gates, it is not permission to defer all tests until the end.
...
export function scryWorld() {
  ...
  const fixture: TownFixture = {
    ...
    npcs: [guard, npc('boss', 'hq', 'clerk'), npc('citizen', 'hall')],
  };
...
describe('scry validates before every mutation', () => {
  it.each([
    ...
    { from: 1380, to: 1455 }, { from: NaN }, { to: Infinity }, { venue: 'missing' }, { tick: 1 },
...
describe('the current tick is the purchased scene', () => {
  it('residue begins once at the window start, even at an empty venue, and persists past its end', () => {
    ...
    expect(world.enemy.evidence.filter((entry) => entry.kind === 'arcane-residue')).toEqual([]);
  ...
  it('offered-tick preview is pure; prior activation and same-tick speech capture have the right order', () => {
    ...
    const events = finishTick(world, R, frame, () => {
      ...
      applyAction(world, { tick: 1440, kind: 'inject', target: 'citizen', spec: {
        subject: 'boss', predicate: 'stole', object: null, count: 2, severity: 4,
        place: null, attribution: 'someone',
      } }, R, frame);
    });
...
The same-tick speech test's nonempty hall emission is an explicit vehicle hypothesis. If that staged inject does not produce an autonomous telling at this base, report the actual frame/selection trace and replace the vehicle with a staged legal `telling` opportunity approved by the controller; never fabricate events for the phase integration assertion or lower a rumor threshold. The direct sensor unit already proves exact event selection independently.
...
import { physicalResidueAt, reportResidue } from '../../src/sim/residue';
...
describe('physical residue waits for lawful discovery and report', () => {
  ...
  it('the two principal copies stay independent and an empty spoken report conveys no residue', () => {
    ...
    expect(stableStringify(world.enemy)).toBe(enemyBefore);
    const atom = { kind: 'arcane-residue' as const, observedAt: 1440, venue: 'hall', residueId: 's0', witness: 'guard' };
    expect(reportResidue(physicalResidueAt(atom))).toEqual(atom);
  });
