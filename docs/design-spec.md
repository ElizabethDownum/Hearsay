...
- **Still open — enemy instrument-audit symmetry (2026-07-05, rule during Plan 8 authoring):** guards' reports already lie by trait; does the v1 enemy digest audit its own instruments the way the player can? Recommendation on the table: v1 enemy stays instrument-blind — that asymmetry is the player's earnable edge (the canny-vs-naive lesson) — and "audits its guards" becomes a spymaster-archetype difficulty knob post-v1.
