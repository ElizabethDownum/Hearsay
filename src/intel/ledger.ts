...
function summaryOf(e: IntelEntry): string {
  ...
  if (e.kind === 'arcane-residue') return 'Physical trace at ' + e.venue;
  return e.kind;
}
...
export function informantLedger(log: readonly IntelEntry[], via: IntelEntry['via']): LedgerView {
  ...
  log.forEach((e) => {
    if (isMagic(e) || e.family === null || e.via === via || !ownFamilies.has(e.family)) return;
